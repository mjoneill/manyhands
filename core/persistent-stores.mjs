...
export const PERSISTENT_STORES = [
  ...
  { match: 'graph-snapshot.json', kind: 'PROJECTION', env: null,
    why: '#884\'s snapshot sidecar (seq, hash cache, triple count) beside graph-snapshot.nq — a dump of the in-process replica keyed by event-log seq, rebuilt by discarding it (the next boot is cold and replays the log). Read by #1386\'s store meter on every /api/checks, which is how the #1152 gate first saw it: the fixture servers never dump one.' },
  { match: 'graph-snapshot.nq', kind: 'PROJECTION', env: null,
    why: 'The replica itself as N-Quads, #884. Same rebuild rule as its sidecar: delete both and the boot is cold.' },
