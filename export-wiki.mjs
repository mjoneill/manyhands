...
import { transformForExport, describeConfig } from './core/export-transforms.mjs';
...
const main = async () => {
  ...
  try {
    ...
    configSource = args.config !== defaultConfigPath()
      ? 'explicit'
  ...
  // ⭐ #1331 — THE ARTIFACT THAT LEAVES THE ROOM MUST SAY WHICH RULESET MADE IT.
  //
  // This exporter hand-rolled its own counts and printed "scrub check PASS —
  // 0 of N forbidden patterns present", which is equally true of an export
  // against a room's 21 rules and of one against the 5 shipped EXAMPLES. A
  // reader could not tell them apart — and this is the exporter whose output
  // LEAVES, while the board exporter makes the archive that stays. The
  // provenance was landing on the wrong side of that asymmetry.
  //
  // ⛔ WHAT IS DELIBERATELY *NOT* CHANGED HERE: the missing-config path still
  // die()s with its two recovery doors. export-board.mjs falls back to generic
  // defaults because it writes an in-room archive; this one hands an artifact
  // to an audience, and "no ruleset was found" is a different question for a
  // thing that leaves. That asymmetry is TESTED and it is the card's own
  // "residue behaviour follows the ARTIFACT, not the script" applied one layer
  // up. I removed it once by over-applying a review note about provenance, and
  // the existing test caught it.
  const provenance = describeConfig(config, { source: configSource, path: args.config });
  ...
  const html = renderExportHtml(scrubbed, {
    ...
    note: `Generated by export-wiki.mjs. Some in-house names are substituted at export; the source retains them. ${provenance.line}`,
  });
  ...
  console.log(`  scrub check   PASS — 0 of ${provenance.forbidden} forbidden patterns present`);
  console.log(`  ruleset       ${provenance.line}`);
  console.log(`  digest        ${digest}  (of the scrubbed CONTENT, not the ruleset)`);
