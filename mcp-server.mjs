...
function buildMcpServer() {
  ...
  mcp.registerTool('card_create', {
    ...
    inputSchema: {
      ...
      acceptance: z.array(z.object({ condition: z.string(), evidence: z.array(z.string()).optional(), note: z.string().optional(), blockedBy: z.array(z.number()).optional() })).optional().describe('Which durable result discharged which release condition. `blockedBy` scopes a blocker to ONE condition instead of the whole card (#1041): shortId numbers, same vocabulary as `relationships.blockedBy`, and the queue reads it as a condition-scoped block rather than a card-level one. Evidence entries must be a full 40-character commit sha or an entity uuid — never a sentence, because prose is what this field replaces and a short sha cannot be expanded by the graph. An undischarged condition still records, with empty evidence, so \'not yet met\' is queryable rather than absent. ⚠️ REPLACES THE WHOLE ARRAY — send every entry you want to keep. Unlike `relationships`, which merges at the type level (#548), this is applied wholesale, so two seats adding different entries seconds apart means the second write SILENTLY DELETES the first, with no error and no merge (#466, measured 2026-08-20 on `blockers`). Omitting the field entirely leaves existing entries untouched — a partial update does not clear what it does not send, so the hazard is CONCURRENT SENDERS, never omission. Read fresh, write once, read back — and on card_update PASS ifVersion: without it the write is REFUSED (#1132), because a read-back against your own snapshot cannot see an entry that arrived while you composed. ⭐ To change ONE entry use the matching *Upsert verb instead (#1137): it sends only that entry, needs no ifVersion, and cannot lose the others.'),
  ...
  mcp.registerTool('card_update', {
    ...
    inputSchema: {
      ...
      ifVersion: z.number().int().nonnegative().optional().describe(
        ...
        + '⛔ REQUIRED with `acceptance`, `blockers` and `checks` (#1132): those REPLACE the whole array and have no append verb, so this is their ONLY defence — a write that sends one of them without ifVersion is refused. For `description` prefer `descriptionAppend`/`descriptionPrepend`, which need no precondition by construction; a full-body `description` replace still benefits from it.',
      ),
      ...
      blockers: z.array(z.object({ card: z.number().optional(), person: z.string().optional(), anyHuman: z.literal(true).optional(), owner: z.string().optional(), status: z.enum(['open','cleared']), note: z.string().optional() })).optional().describe('What is blocking this card, and whether it still is. Each entry names EITHER a `card` (which must already be in this card\'s blockedBy — ownership describes an existing edge, it does not create one; `owner` is who is chasing it) OR a `person` (whose own pending action IS the block, and which needs no edge because the blocker is not a card) OR `anyHuman: true` (#966 — ANY human participant can clear it; it names nobody, so a query for a NAMED person cannot match it. `anyHuman: false` is refused: absence must not masquerade as a decision). Naming more than one is refused: the two are opposite states and an entry meaning either cannot be read as one. Projects as scrum:Blocker nodes with scrum:blockedByCard / scrum:blockedByPerson, so \'what is waiting on me\' is ONE query instead of a regex over prose. ⚠️ REPLACES THE WHOLE ARRAY — send every entry you want to keep. Unlike `relationships`, which merges at the type level (#548), this is applied wholesale, so two seats adding different entries seconds apart means the second write SILENTLY DELETES the first, with no error and no merge (#466, measured 2026-08-20 on `blockers`). Omitting the field entirely leaves existing entries untouched — a partial update does not clear what it does not send, so the hazard is CONCURRENT SENDERS, never omission. Read fresh, write once, read back — and on card_update PASS ifVersion: without it the write is REFUSED (#1132), because a read-back against your own snapshot cannot see an entry that arrived while you composed. ⭐ To change ONE entry use the matching *Upsert verb instead (#1137): it sends only that entry, needs no ifVersion, and cannot lose the others.'),
      checks: z.array(z.object({ claim: z.string(), ask: z.string(), expect: z.boolean() })).optional().describe('Falsifier tripwires for this card\'s load-bearing claims. Each is {claim, ask, expect}: the sentence in your own words, a SPARQL ASK whose answer would falsify it, and the boolean it must return today. GET /api/checks runs them all and reports holds/stale/error. Attach one when you write a claim about what exists — that is the only moment you know what would make it false. ASK only: a SELECT returns rows, not a boolean, so the check could never fail. ⚠️ REPLACES THE WHOLE ARRAY — send every entry you want to keep. Unlike `relationships`, which merges at the type level (#548), this is applied wholesale, so two seats adding different entries seconds apart means the second write SILENTLY DELETES the first, with no error and no merge (#466, measured 2026-08-20 on `blockers`). Omitting the field entirely leaves existing entries untouched — a partial update does not clear what it does not send, so the hazard is CONCURRENT SENDERS, never omission. Read fresh, write once, read back — and on card_update PASS ifVersion: without it the write is REFUSED (#1132), because a read-back against your own snapshot cannot see an entry that arrived while you composed. ⭐ To change ONE entry use the matching *Upsert verb instead (#1137): it sends only that entry, needs no ifVersion, and cannot lose the others.'),
      acceptanceUpsert: z.array(z.object({ condition: z.string(), evidence: z.array(z.string()).optional(), note: z.string().optional(), blockedBy: z.array(z.number()).optional(), replaces: z.string().optional() })).optional().describe('#1137 — insert-or-replace ONE OR MORE acceptance entries, matched on `condition` (exact text). Send only the entries you are changing; every other entry is untouched by construction, so no ifVersion is needed (\u26a0\ufe0f #1169 — unless two existing entries share the same `condition` text, in which case the upsert is REFUSED rather than rewriting one of the pair) and two seats upserting different conditions seconds apart BOTH survive. A whole-entry replace: send the full entry (evidence, note, blockedBy) as you want it. Cannot be combined with `acceptance`. PREFER THIS to `acceptance` for discharging a condition. ⚠️ #1158 — because the key is EXACT TEXT, a REWORDED condition would insert a twin and leave the original with its old evidence: to reword, send `replaces: <old text exactly as stored>` and the entry is renamed in place with its evidence kept; a near-identical condition sent without `replaces` is REFUSED naming the twin. \u26a0\ufe0f #1169 \u2014 `replaces` is the ONE path the duplicate guard does NOT cover: if two existing entries share the old text it renames the FIRST and leaves the second under it. Nothing is destroyed and both survive with their own evidence, but the choice is the schema\u2019s, not yours \u2014 and you cannot see the pair from here, because `card_get` shows both entries and flags neither.'),
      blockersUpsert: z.array(z.object({ card: z.number().optional(), person: z.string().optional(), anyHuman: z.literal(true).optional(), owner: z.string().optional(), status: z.enum(['open','cleared']), note: z.string().optional() })).optional().describe('#1137 — insert-or-replace blocker entries, matched on their target: `card` N, `person` X, or the single `anyHuman` entry. Same validation as `blockers` (a card blocker must be in this card\'s blockedBy). Untouched entries are never sent, so they cannot be lost BY OMISSION; no ifVersion needed. \u26a0\ufe0f #1169 — the key is the TARGET, and nothing makes a target unique: two entries can both name `person: X`. When one does, the upsert is REFUSED naming the count, because resolving to the first would rewrite that entry whole and leave its twin untouched, with a 200. Send the full `blockers` array with `ifVersion` to fix such a pair. Cannot be combined with `blockers`. PREFER THIS to `blockers` for clearing one blocker.'),
      checksUpsert: z.array(z.object({ claim: z.string(), ask: z.string(), expect: z.boolean() })).optional().describe('#1137 — insert-or-replace check entries, matched on `claim` (exact text). ASK only, as for `checks`. Untouched entries are never sent, so they cannot be lost BY OMISSION; no ifVersion needed. \u26a0\ufe0f #1169 — if two existing entries share a `claim`, the upsert is REFUSED rather than rewriting one of them; send the full `checks` array with `ifVersion`. Cannot be combined with `checks`. PREFER THIS to `checks` for adding or re-authoring one tripwire.'),
      implementedBy: z.array(z.string()).optional().describe('FULL 40-char git shas of the commits implementing this card. Short shas are refused: the graph cannot expand an abbreviation, so both forms would become two nodes for one commit. Makes "what implements #N" and "what did this commit implement" one-hop queries instead of prose archaeology.'),
      relationships: z.object({
  ...
  mcp.registerTool('conversation_post', {
    ...
    inputSchema: {
      ...
      conversation: z.string().optional().describe('#1401 — the 1:1 talk this post belongs to (a talk id from a "Talk with…" wake or from talk_list). The post stays board-level and visible to the whole room; the tag is what the talk\'s view filters by. If you were handed a talk id in the message you are answering, reply with the same id so your answer lands in that view. Names no talk → REFUSED; names a CLOSED talk → 409 TALK_CLOSED until a participant reopens it. #1409 NORM (the owner\'s word, 2026-09-18): inside a talk the seat the human is WITH is the concierge — if you are not that seat, answer in the room (no tag) and let the concierge fold in what matters; tag a post into someone else\'s talk only on purpose. The tag is never inherited by default.'),
    },
  }, async (args, extra) => {
  ...
  mcp.registerTool('graph_query', {
    ...
    description: '⚠️ SILENT FAILURE IS THIS TOOL\'S DEFAULT FAILURE MODE: a wrong predicate, '
      ...
      + 'CARD TYPE — `?c a scrum:Card` and `?c a schema:CreativeWork` BOTH match every '
      + 'card, and either is fine to write (#962). It did not used to be: the guessable '
      + 'spelling returned ZERO ROWS AND NO ERROR while 933 cards existed, so the seat '
      + 'writing the obvious query got a confident silent lie and the seat who knew the '
      + 'arcane spelling got the answer. '
      + '⚠️ scrum:Card is PROJECTION-ONLY: the stored document types a card as '
      + 'CreativeWork alone, so an export or a direct read of board-data.json will NOT '
      + 'show it. Two surfaces, one name — query the graph and the file and you get '
      + 'different type sets, by design and not by accident. '
      + 'A silent 0 is '
