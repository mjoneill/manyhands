...
function buildMcpServer() {
  ...
  mcp.registerTool('card_create', {
    ...
    inputSchema: {
      ...
      acceptance: z.array(z.object({ condition: z.string(), evidence: z.array(z.string()).optional(), note: z.string().optional() })).optional().describe('Which durable result discharged which release condition. Evidence entries must be a full 40-character commit sha or an entity uuid — never a sentence, because prose is what this field replaces and a short sha cannot be expanded by the graph. An undischarged condition still records, with empty evidence, so \'not yet met\' is queryable rather than absent.'),
      blockers: z.array(z.object({ card: z.number().optional(), person: z.string().optional(), owner: z.string().optional(), status: z.enum(['open','cleared']), note: z.string().optional() })).optional().describe('What is blocking this card, and whether it still is. Each entry names EITHER a `card` (which must already be in this card\'s blockedBy — ownership describes an existing edge, it does not create one; `owner` is who is chasing it) OR a `person` (whose own pending action IS the block, and which needs no edge because the blocker is not a card). Naming both is refused: the two are opposite states and an entry meaning either cannot be read as one. Projects as scrum:Blocker nodes with scrum:blockedByCard / scrum:blockedByPerson, so \'what is waiting on me\' is ONE query instead of a regex over prose.'),
