...
function buildMcpServer() {
  ...
  mcp.registerTool('card_create', {
    ...
    inputSchema: {
      ...
      blockers: z.array(z.object({ card: z.number(), owner: z.string().optional(), status: z.enum(['open','cleared']), note: z.string().optional() })).optional().describe('Who owns clearing each blocker, and whether they still do. Each entry must name a card already in this card\'s blockedBy — ownership describes an existing edge, it does not create one. Projects as scrum:Blocker nodes so \'each blocker\'s owner and status\' is one graph query instead of prose.'),
