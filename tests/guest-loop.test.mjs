...
test('#1201 the runnable form exists and uses the loop: scripts/guest-once.mjs imports guestOnce and findMentions', () => {
  ...
  assert.match(src, /import \{ findMentions, findWakes, guestOnce, fetchBoundedChanges, shouldMarkAnswered, mentionScanPath, fetchMentionWindow, acquireLock, releaseLock, effectiveWakeOn, budgetCheck \} from '\.\.\/core\/guest-loop\.mjs'/);   // #1237 widened the import; #1274 added the paged window; #1346 the channel drain
