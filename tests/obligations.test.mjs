...
const STEWARD = { by: 'ada', owedBy: 'ada', about: 1086, kind: 'steward', note: 'Value Steward on slice 2 — not builder' };
...
test('#1118-A create → list → read back: an obligation is a node with a holder, a subject, a kind and an open status', async () => {
  ...
  try {
    ...
    assert.equal(c.body.owedBy, 'ada');
    ...
    const mine = await json(s.baseUrl, 'GET', '/api/obligations?owedBy=ada&status=open');
    assert.equal(mine.body.length, 1, '"what do I owe, open" is one filtered read');
    const none = await json(s.baseUrl, 'GET', '/api/obligations?owedBy=bo');
...
test('#1118-A refusals name what to do: missing by, unknown kind, a subject that names no node', async () => {
  ...
  try {
    ...
    const noHolder = await json(s.baseUrl, 'POST', '/api/obligations', { ...STEWARD, owedBy: '' });
    assert.equal(noHolder.status, 400); assert.match(noHolder.body.error, /owedBy/i);
  } finally { await s.stop(); }
});
...
test('#1118-A ANY NODE: an obligation may be about a decision or a predicate, not only a card', async () => {
  ...
  try {
    ...
    const c = await json(s.baseUrl, 'POST', '/api/obligations', { by: 'bo', owedBy: 'bo', about: pid, kind: 'review', note: 'write the honest definition' });
    assert.equal(c.status, 201, JSON.stringify(c.body));
...
test('#1118-A discharge: PATCH status=discharged stamps who and when; a discharged one leaves the open set; re-discharging is a noop', async () => {
  ...
  try {
    ...
    const open = await json(s.baseUrl, 'GET', '/api/obligations?owedBy=ada&status=open');
    ...
    // a closure learned about LATER carries its real time, not a plausible false now
    const c2 = await json(s.baseUrl, 'POST', '/api/obligations', { ...STEWARD, note: 'second' });
    const back = await json(s.baseUrl, 'PATCH', `/api/obligations/${encodeURIComponent(c2.body.id)}`, { by: 'bo', status: 'lapsed', dischargedAt: '2026-08-01T00:00:00Z' });
    assert.equal(back.status, 200, JSON.stringify(back.body));
    assert.equal(back.body.dischargedAt, '2026-08-01T00:00:00.000Z');
    const c3 = await json(s.baseUrl, 'POST', '/api/obligations', { ...STEWARD, note: 'third' });
    const badTime = await json(s.baseUrl, 'PATCH', `/api/obligations/${encodeURIComponent(c3.body.id)}`, { by: 'bo', status: 'lapsed', dischargedAt: 'yesterday-ish' });
    assert.equal(badTime.status, 400);
  } finally { await s.stop(); }
});

test('#1118-A BORN IN THE GRAPH: owedBy is a person EDGE, about is an entity EDGE, and "what does ada owe, open" is one query', async () => {
  const s = await startRestServer({ board: board() });
  try {
    const c = await json(s.baseUrl, 'POST', '/api/obligations', STEWARD);
    ...
    const q = await json(s.baseUrl, 'POST', '/api/graph', {
      by: 'ada',
      query: 'SELECT ?o ?about ?kind ?status ?note WHERE { ?o a scrum:Obligation ; scrum:owedBy person:ada ; schema:about ?about ; scrum:obligationKind ?kind ; scrum:status ?status ; schema:text ?note }',
    });
    ...
    const j = await json(s.baseUrl, 'POST', '/api/graph', {
      by: 'ada',
      query: 'SELECT ?title WHERE { ?o a scrum:Obligation ; scrum:owedBy person:ada ; schema:about ?c . ?c schema:name ?title }',
    });
...
test('#1118-A ⭐ REACHABLE BY AN MCP-ONLY SEAT — create, list, close, all through the tools (#904, not repeated)', async () => {
  ...
  try {
    ...
    const pid = 'https://scrumboard.local/predicate/scrum%3ArelatedTo';
    const c = await session.callTool('obligation_create', { by: 'ada', owedBy: 'bo', about: pid, kind: 'review', note: 'define it honestly' });
    ...
    const open = await session.callTool('obligation_list', { owedBy: 'bo', status: 'open' });
    assert.equal(JSON.parse(open.result.content[0].text).length, 1, 'an MCP seat can ask "what do I owe, open"');
    ...
    const none = await session.callTool('obligation_list', { owedBy: 'bo', status: 'open' });
